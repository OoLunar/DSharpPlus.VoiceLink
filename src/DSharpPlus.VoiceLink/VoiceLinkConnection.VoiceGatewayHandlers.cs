...
namespace DSharpPlus.VoiceLink
{
    public sealed partial class VoiceLinkConnection
    {
        ...
        private static async ValueTask SpeakingAsync(VoiceLinkConnection connection, ReadResult result)
        {
            ...
            {
                voiceLinkUser = new(connection, voiceSpeakingPayload.Ssrc, await connection.Guild.GetMemberAsync(voiceSpeakingPayload.UserId), connection._audioDecoderFactory(connection.Extension.Client.ServiceProvider));
